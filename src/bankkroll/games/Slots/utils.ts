...
export const getSlotCombination = (
  ...
  if (multiplier > 0) {
    ...
    const item =
      pickRandom(items) ??
      pickRandom(SLOT_ITEMS.filter((x) => x.multiplier < 1));
  ...
  const { items } = Array.from({ length: count }).reduce<{
    ...
    ({ previous, items }, _, i) => {
      const item = (() => {
        ...
        if (i === count - 1) {
          return (
            pickRandom(availableSlotItems.filter((x) => !items.includes(x))) ??
            pickRandom(SLOT_ITEMS)!
          );
        }
